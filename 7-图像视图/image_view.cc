...
private:
    ...
    // 为交换链中每一个VkImage创建一个图像视图VkImageView
    ...
    void createImageViews() {
        ...
        // 为交换链中的每一个图像建立图像视图。首先，添加用于存储图像视.
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            VkImageViewCreateInfo createInfo{};
            createInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
            createInfo.image = swapChainImages[i]; // 交换链图像
            createInfo.viewType = VK_IMAGE_VIEW_TYPE_2D; // 一维纹理、二维纹理、三维纹理还是立方体贴图。
            createInfo.format = swapChainImageFormat; // 交换链图像格式
