...
private:
    ...
    std::vector<VkCommandBuffer> commandBuffers ; // 指令缓冲区数组，由于绘制操作是在帧缓冲上进行的，我们需要为交换链中的每一个图像分配一个指令缓冲对象。
    VkCommandBuffer commandBuffer; // 指令缓冲区
    ...
    void createCommandPool() {
        ...
        // sType 和 flags：影响的是 命令缓冲（Command Buffer） 的内存分配和重置策略。
        // VK_COMMAND_POOL_CREATE_TRANSIENT_BIT：告诉驱动：“我会从这个 Command Pool 分配很多 短命 的 Command Buffer。”
        // VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT：告诉 Vulkan：“我希望可以单独重置某个 Command Buffer，而不是一次性重置整个 Command Pool。”
        // 例如：
        // 一个 Command Buffer 专门渲染 UI（每帧更新）
        // 另一个 Command Buffer 渲染静态背景（几秒才更新一次）
        poolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO; // 表示创建一个指令池
        poolInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT; // 表示创建一个指令缓冲区
        poolInfo.queueFamilyIndex = queueFamilyIndices.graphicsFamily.value(); // 图形队列族索引
        // 疑问？
        // 创建指令池时，为什么需要指定图形队列族索引？而不用呈现队列所引？
        // 答案：
        // 图形队列处理的操作：----------------------- 呈现队列处理的操作：
        // - 顶点着色器执行                          - 将渲染完成的图像提交给显示器
        // - 片段着色器执行                          - 交换链图像管理
        // - 光栅化                                 - 垂直同步（VSync）
        // - 深度测试                               - 显示时序控制
        // - 颜色混合
    ...
    void createCommandBuffer() {
        ...
        allocInfo.commandBufferCount = 1; // 表示创建一个指令缓冲区，可以创建多个指令缓冲区
